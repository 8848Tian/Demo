...
static void MODS_01H(void)
{
	...
	*/
    ...
    m = (num + 7) / 8;
    //寄存器号大于等于开始地址，寄存器个数大于0，寄存器号+个数在所有寄存器的地址范围内
...
static void MODS_05H(void)
{
	...
	*/
    ...
    //寄存器地址在范围内
    if ((reg >= REG_D01) && (reg <= REG_DXX))
    {
        if (value)
            temp[(reg - REG_D01) / 8] |= (1 << ((reg - REG_D01) % 8));
        else
            temp[(reg - REG_D01) / 8] &= ~(1 << ((reg - REG_D01) % 8));
    }
    else
...
static void MODS_0fH(void)
{
    ...
    //寄存器号大于等于开始地址，寄存器个数大于0，寄存器号+个数在所有寄存器的地址范围内
    if ((reg_addr >= REG_D01) && (reg_num > 0) && (reg_addr + reg_num <= REG_DXX + 1))
    {
        ...
        {
            //地址在范围内
            if ((reg_addr + i) <= (REG_DXX + 1))
            {
                if (BUF[i / 8] & (1 << (i % 8)))
                    temp[(reg_addr + i - REG_D01) / 8] |= (1 << (i % 8));
                else
                    temp[(reg_addr + i - REG_D01) / 8] &= ~(1 << (i % 8));
            }
            else
